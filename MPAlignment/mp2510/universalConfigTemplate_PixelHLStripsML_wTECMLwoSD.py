...
tagwriter.setCondition(process,
       ...
       tag = "TrackerAlignment_StartUp17_v10")
tagwriter.setCondition(process,
       connect = "frontier://FrontierProd/CMS_CONDITIONS",
       record = "TrackerSurfaceDeformationRcd",
       tag = "TrackerSurfaceDeformations_StartUp17_v9")
# we start from global tag so we do not need that
#tagwriter.setCondition(process,
      #connect = "sqlite_file:/afs/cern.ch/cms/CAF/CMSALCA/ALCA_TRACKERALIGN/MP/MPproduction/mp2429/jobData/jobm/alignments_MP.db",
      #record = "TrackerAlignmentRcd",
      #tag = "Alignments")
...
process.AlignmentProducer.ParameterBuilder.SelectorBowed = cms.PSet(
    alignParams = cms.vstring(
        "TrackerP1PXBModule,111111 000", # only position wo surface def
        "TrackerP1PXECModule,110111 000", # w coordinate is third 1 
