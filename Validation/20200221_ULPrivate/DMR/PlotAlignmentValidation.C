...
//------------------------------------------------------------------------------
void PlotAlignmentValidation::plotDMR(const std::string& variable, Int_t minHits, const std::string& options, const string& date)
{
  ...
  if ( findres != std::string::npos) {
    ...
    plotDMR(substring1, minHits, options, date);
    plotDMR(substring2, minHits, options, date);
    return;
   }
  ...
  if (variable == "mean" || variable == "median" || variable == "meanNorm" ||
    ...
    plotDMR(variable+"X", minHits, options, date);
    plotDMR(variable+"Y", minHits, options, date);
    return;
  }
  ...
  if (variable == "meanX") {          plotinfo.nbins = 50;  plotinfo.min = -0.001; plotinfo.max = 0.001; }
    ...
    else {                            plotinfo.nbins = 50;  plotinfo.min = -0.001; plotinfo.max = 0.001; } //KELLO override from nbins=100
  ...
  else if (variable == "rmsX") {      plotinfo.nbins = 50; plotinfo.min = 0.0;    plotinfo.max = 0.1; } //KELLO override from nbins=100
  else if (variable == "rmsY") {      plotinfo.nbins = 100; plotinfo.min = 0.0;    plotinfo.max = 0.1; }
  else if (variable == "rmsNormX") {  plotinfo.nbins = 50; plotinfo.min = 0.3;    plotinfo.max = 1.8; } //KELLO override from nbins=100
  ...
  for (int i=1; i<=6; ++i) {
	...
	if (plotinfo.plotSplits) {
	  ...
	  if (plotinfo.h1 != 0 && plotinfo.h2 != 0 && !plotinfo.plotPlain) {
	    ...
	    float deltamuerror = factor*sqrt(pow(plotinfo.h2->GetMeanError(1), 2) + pow(plotinfo.h1->GetMeanError(1), 2));
    ...
    //float H = pad->GetWh();
    //float W = pad->GetWw();
    //float l = gStyle->GetPadLeftMargin()
    //float r = gStyle->GetPadRightMargin()
    //float t = gStyle->GetPadTopMargin()
    //float b = gStyle->GetPadBottomMargin()
    //TLatex header;
    //"#CMS{Preliminary}" 
    //header.SetNDC();
    //header.SetTextAngle(0);
    //header.SetTextColor(kBlack);    
    //header.SetTextFont(61);
    //header.SetTextAlign(11); 
    //header.SetTextSize(0.75);
    //r     
    //header.DrawLatex(1-r,1-t+lumiTextOffset*t,lumiText);

    TPaveText *header = TkAlStyle::standardTitle(PRELIMINARY);
    header->Draw();
    //TPaveText *header_r = TkAlStyle::customRightTitle("(2016+2017+2018 pp collisions)");
    TPaveText *header_r = TkAlStyle::customRightTitle(date.c_str()); //KELLO override 
    header_r->Draw();
    ...
    double st_x = 0.7;
    double st_y = 0.7;
    datePT = new TPaveText(st_x, st_y, st_x+0.15, st_y+0.1, "brNDC");
    //datePT->SetTextSize(0.06);
    //datePT->SetTextFont(8);
    datePT->SetFillStyle(0);
    datePT->SetBorderSize(0);
    datePT->SetMargin(0.01);
    datePT->SetTextAlign(13); // align left
    datePT->AddText(date.c_str());
    //datePT->Draw(); KELLO OVERRIDE
    ...
    if (vdeltameanerror.size()) {
      summaryfile << "  sigma_dmu_" << subdet ;
      if (plotinfo.variable == "medianY") summaryfile << "_y";
      summaryfile << " (um)\t"
                  << "latexname=$\\sigma\\Delta\\mu_\\text{" << subdet << "}";
      if (plotinfo.variable == "medianY") summaryfile << "^{y}";
      summaryfile << "$ ($\\mu$m)\t"
                  << "format={:.3g}\t"
                  << "latexformat=${:.3g}$";
      for (auto dmu : vdeltameanerror) summaryfile << "\t" << dmu;
      summaryfile << "\n";
    }
    if (vmeanerror.size()) {
